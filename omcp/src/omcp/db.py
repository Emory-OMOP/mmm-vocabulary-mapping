...
class OmopDatabase:
    ...
    def __init__(
        ...
        try:
            ...
            # Release immediately — we'll reconnect per-query for DuckDB
    ...
    def _release_connection(self):
        """Release the database connection to free file locks.

        For DuckDB, this is essential: DuckDB's lock model is all-readers
        OR single-writer, so a persistent connection (even read-only) blocks
        other processes from writing. Connect-per-query avoids this.
        """
        if self.target_dialect == "duckdb" and self._conn is not None:
            with self._conn_lock:
                try:
